...
def addOrUpdateAsset(org_id, orgObject):
    ...
    # Retrieve all EC2 instances currently in the database
    all_ec2_objs = Ec2.objects.all()

    # Create a list of EC2 instance IDs in the database
    db_instances = [i.ec2_id for i in all_ec2_objs]

    # Create a list of EC2 instance IDs from AWS
    aws_instances = [instance['id'] for instances in allInstances.values() for instance in instances]

    # Find instances that are in the database but not in AWS
    deleted_instances = set(db_instances) - set(aws_instances)

    # Handle instances that are in the database but not in AWS
